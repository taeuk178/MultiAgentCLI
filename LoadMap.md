...
**문서 책임**
- 본 문서는 **큰 그림 문서**입니다. 제품 방향, 아키텍처, 단계별 로드맵, 장기 위험 요소를 정리합니다.
- 다음 세션에서 바로 볼 단기 체크리스트와 운영 관찰 항목은 `HANDOFF.md` 를 봅니다.
- 결정 사유와 폐기한 대안은 `HISTORY.md` 를 봅니다.
- 현재 구현 동작, 설치, 사용법, 전체 플로우 다이어그램은 `README.md` 를 봅니다.
- hook 단계별 시스템 의존과 운영 환경 변수는 `flow.md` 를 봅니다.
...
사용자 입력
  -> UserPromptSubmit hook
       최근 memory 조회, routing advisory, context prepend
  -> Claude Code 응답
  -> Stop hook
       응답 archive, durable memory 추출
  -> 다음 turn
       저장된 기억이 다시 prefill/search/retrieve 후보가 됨
```

API key 없이 Claude Code OAuth 구독을 그대로 사용합니다. 무거운 LLM 작업(prefill 분석, Slack/Notion fetch, response extract)은 background 에서 `claude -p haiku` 로 분리합니다.
...
3. **근거 있는 답변**
   모델이 기억을 “느낌상” 말하는 것이 아니라, 사용자가 `/memory show`, `/memory inject`, `/retrieve` 로 근거 chunk 를 확인할 수 있어야 합니다.
...
- `UserPromptSubmit`: prompt redaction, `events.user_message` 저장, routing rule 평가, `memory_chunks` prefill, lazy-fetch worker spawn.
...
`memory_chunks` 는 기본 사용자 RAG 기억입니다. Stop hook 추출, external lazy-fetch, `/memory remember` 가 여기에 저장합니다. 다음 turn prefill, `/memory search/list/show/inject`, `/retrieve` fallback 이 이 테이블을 읽습니다.
...
- local: chunk retrieval → RRF → BOOST → memory fallback gate → rerank/CTX.
- feature/global: summary 검색 + chunk retrieval + grounding + contradiction check.
- 문서 후보가 0개면 `memory_chunks` 를 read-only fallback 으로 조회합니다.
- `source_status` marker 는 답변 후보에서 제외합니다.

현재 정확한 플로우는 `README.md` 의 Mermaid 를 기준으로 합니다.
...
기존 자동 hook 의 `memory_chunks` 직접 INSERT 경로는 queue 를 거치지 않습니다. WAL + busy_timeout 으로 일반 동시성은 흡수하고, 필요해질 때만 통합을 검토합니다.

## 현재 기준선

2026-05-16 기준 RAG 기본 기능 1차 기준선은 적용 완료입니다.

- redaction coverage.
- hook memory loop smoke test.
- `/memory` 기본 검색/list/show/inject/remember.
- 한국어 2자 토큰 fallback.
- external source 상태 가시화.
- events noise soft flag.
- `/memory profile`.
- `/retrieve` memory fallback.
- README 전체 플로우 정합성 보정.
...
- `source_status` marker TTL 또는 dedupe.
- noise row 감쇠/삭제 여부.
...
목표는 현재 staged diff, 최근 memory, 테스트 결과를 결합해 개발 워크플로 산출물을 만드는 것입니다.
...
- chunk lifecycle 정책(dedup, 자동 pin, ranking 가중치).
...
- **문서 retrieval 과 memory write 경로 분리**: 지금은 저장 경로를 섞지 않고, `/retrieve` fallback 으로 읽기 정합성만 보장합니다.
