...
```bash
python3 scripts/imprint/sync-plugin-version.py 0.1.2
git add VERSION plugin.json .claude-plugin .codex-plugin .agents/plugins/marketplace.json
git commit -m "plugin 배포 버전 0.1.2 동기화"
git push origin main
git tag 0.1.2
git push origin 0.1.2
gh release create 0.1.2 --title "imprint 0.1.2" --notes-file /tmp/imprint-0.1.2-release.md
```
...
```bash
codex plugin marketplace add taeuk178/imprint --ref 0.1.2
codex plugin marketplace upgrade
```

이미 설치된 plugin을 `0.1.2` 기준으로 제거 후 다시 추가하려면 아래 순서로 실행합니다.
