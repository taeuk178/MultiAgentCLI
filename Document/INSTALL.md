...
git commit -m "plugin 배포 버전 0.1.1 동기화"
git push origin main
git tag 0.1.1
git push origin 0.1.1
gh release create 0.1.1 --title "imprint 0.1.1" --notes "Claude Code/Codex memory plugin release"
```
...
```bash
codex plugin marketplace add taeuk178/imprint --ref 0.1.1
codex plugin marketplace upgrade
```

이미 설치된 plugin을 `0.1.1` 기준으로 제거 후 다시 추가하려면 아래 순서로 실행합니다.
