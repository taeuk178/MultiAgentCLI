"""contradiction detection — 후보 생성 + NLI 판정 skeleton + 3구간 분기.

후보 생성: same entity + decision 타입 + same scope_key/section_path + time gap < 90d.
NLI: mDeBERTa-v3-base-mnli-xnli lazy 로드. 미가용 시 후보를 status=candidate 로 그대로 저장 (자동 dismiss 금지).
"""
...
def _nli_score(premise: str, hypothesis: str) -> tuple[float, str] | None:
    ...
    def _run() -> None:
        try:
            ...
            holder["reason"] = f"contradiction={scores.get('contradiction',0):.2f} entailment={scores.get('entailment',0):.2f}"
        except Exception as exc:
...
def scan_and_store(
    ...
    *,
    ...
    """후보 생성 → NLI 판정 → contradictions 테이블에 저장.
    ...
    try:
        ...
        for c in candidates:
            ...
            detector = "nli" if use_nli and is_nli_available() else "rule"
            ...
            if detector == "nli":
                nli_out = _nli_score(c.a_text, c.b_text)
                if nli_out is None:
                    detector = "rule"  # fallback
                    stats.nli_skipped += 1
                else:
                    score, reason = nli_out
                    stats.nli_used += 1
            if detector == "rule":
                # rule 기반 점수 — 두 청크 텍스트 불일치만으로 약 신호.
                score = 0.5 if c.a_text != c.b_text else 0.0
                reason = "rule: text differs"
            ...
            if existing:
                conn.execute(
                    ...
                    WHERE id = ?
                    """,
                    (score, status, reason, ts, existing["id"]),
                )
            else:
                cid = new_id()
                conn.execute(
                    ...
                    (cid, project_id, c.entity_id, c.scope_key,
                     a_id, b_id, score, detector, status, reason, ts, ts),
                )
