...
def _feature_key_for_chunk(row: sqlite3.Row) -> str | None:
    ...
    1순위: entry_entities 의 canonical_name (가장 confidence 높은 entity).
...
def _select_chunks_for_feature(conn: sqlite3.Connection, project_id: str, feature_key: str) -> list[sqlite3.Row]:
    ...
    매칭 룰: entry_entities.canonical_name 이 feature_key 의 ":" 뒤 부분과 일치하거나,
    ...
    cur = conn.execute(
        """
        SELECT c.id, c.text, c.retrieval_text, c.section_path, c.normalized_type,
               c.is_current, c.source_updated_at,
               d.title AS document_title, d.source_type, d.id AS source_document_id,
               (SELECT e.canonical_name FROM entry_entities ce
                JOIN entities e ON e.id = ce.entity_id
                WHERE ce.entry_id = c.id
...
def _deterministic_summarize(texts: list[str], *, max_lines: int = DETERMINISTIC_MAX_LINES) -> str:
...
def _upsert_summary(
    ...
    *,
    ...
    conn.execute(
        """
        UPDATE search_summaries SET valid_to = ?, is_current = 0
    ...
    *,
    ...
    try:
        ...
        if not chunks:
            ...
            conn.execute(
                """
                UPDATE search_summaries SET is_current = 0, valid_to = ?
        ...
        if not summary_text:
            summary_text = _deterministic_summarize([r["text"] for r in chunks])
        ...
        conn.execute(
            "UPDATE search_summaries SET source_chunk_count = ? WHERE id = ?",
            (len(chunks), sid),
        )
        children = [("entry", r["id"], 1.0) for r in chunks]
...
def _document_feature_keys(conn: sqlite3.Connection, project_id: str, source_document_id: str) -> list[str]:
    ...
    cur = conn.execute(
        ...
        FROM search_entries c
        JOIN source_documents d ON d.id = c.source_document_id
        WHERE c.project_id = ? AND c.source_document_id = ? AND c.is_current = 1
        """,
...
def regenerate_document(
    ...
    *,
    ...
    if own:
        conn = db_connect()
    try:
        feature_keys = _document_feature_keys(conn, project_id, source_document_id)
        feature_summary_ids: list[str] = []
        for fk in feature_keys:
            cur = conn.execute(
                """
                SELECT id FROM search_summaries
                WHERE project_id = ? AND level = 'feature' AND target_key = ? AND is_current = 1
                """,
                (project_id, fk),
            )
        ...
        if not feature_summary_ids:
            cur = conn.execute(
                "SELECT id, text FROM search_entries WHERE project_id = ? AND source_document_id = ? AND is_current = 1",
                (project_id, source_document_id),
            )
            ...
            if not chunk_rows:
                conn.execute(
                    ...
                    """,
                    (now_iso(), project_id, f"document:{source_document_id}"),
                )
                return None
            text = _deterministic_summarize([r["text"] for r in chunk_rows])
            sid = _upsert_summary(
                conn, project_id=project_id, level="document",
                target_key=f"document:{source_document_id}",
                title=None, summary_text=text, generator="deterministic",
            )
            _replace_links(conn, sid, [("entry", r["id"], 1.0) for r in chunk_rows])
            return sid

        # feature summary 들의 본문을 모아 document summary 작성.
        cur = conn.execute(
            f"SELECT id, summary_text FROM search_summaries WHERE id IN ({','.join('?' * len(feature_summary_ids))})",
            tuple(feature_summary_ids),
        )
        feat_rows = cur.fetchall()
        prompt = (
            f"Summarize the following feature search_summaries into a document-level summary in Korean.\n"
        ...
        conn.execute(
            "UPDATE search_summaries SET source_summary_count = ? WHERE id = ?",
            (len(feat_rows), sid),
        )
...
def regenerate_project(
    ...
    *,
    ...
    try:
        cur = conn.execute(
            """
            SELECT id, summary_text FROM search_summaries
        ...
        prompt = (
            "Summarize the following document search_summaries into a project-level summary in Korean.\n"
...
def regenerate_for_document(
    ...
    *,
    ...
    try:
        ...
        sid = regenerate_document(project_id, source_document_id, use_llm=use_llm, conn=conn)
        if sid:
...
def list_drilldown_chunks(
    ...
    """summary 의 child_kind=entry link 1~3개 조회 — GROUND 단계 grounding."""
    own = conn is None
    if own:
        conn = db_connect()
    try:
        cur = conn.execute(
            """
            SELECT c.id, c.text, c.section_path, c.is_current, c.source_updated_at,
            ...
            WHERE sl.parent_summary_id = ? AND sl.child_kind = 'entry'
