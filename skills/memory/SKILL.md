...
- 기본 사용자 RAG 경로는 자동 prefill + `/memory search`/`inject` 입니다. `/search` 는 별도 `documents`/`chunks_v2`/`summaries` 기반 문서 검색 경로를 먼저 사용하고, 문서 후보가 없을 때 `memory_chunks`를 read-only fallback 으로 조회합니다.
