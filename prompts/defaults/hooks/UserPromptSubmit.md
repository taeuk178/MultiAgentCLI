...
무엇을 함:
  ...
  3) gate 결과에 따라 lane 별 메모리 청크를 [Project memory context] 블록으로 prepend
...
3. **메모리 주입** — current clues, recent session evidence, durable/external evidence 를 `[Project memory context]` 블록으로 prepend
