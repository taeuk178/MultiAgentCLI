...
pub fn spawn_config(provider: &ProviderId) -> SpawnConfig {
    provider_for(*provider).spawn_config()
}

pub fn run_chat(provider: &ProviderId, prompt: &str, cwd: Option<&str>) -> Result<String, String> {
    let mut command = provider_for(*provider).chat_command(prompt);
...
pub fn runtime_statuses() -> Vec<ProviderRuntimeStatus> {
    [ProviderId::Claude, ProviderId::Codex, ProviderId::Gemini]
        .into_iter()
        .map(|provider| {
            let implementation = provider_for(provider);
            let healthy = implementation.command_exists();
            let usage = implementation.runtime_usage();
            ProviderRuntimeStatus {
                model: implementation.configured_model(),
